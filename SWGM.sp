...
public Plugin myinfo =
{
	...
	author = "Someone",
	description = "Дополнительные возможности для плагинов",
	version = "1.7",
	url = "http://hlmod.ru"
};
...
public void OnPluginStart()
{
	...
	char sBuffer[14];
	(CVAR = CreateConVar("swgm_groupid",		"XXXXXXXX",	"ID Steam группы.")).AddChangeHook(OnGroupChange);
	CVAR.GetString(sBuffer, sizeof(sBuffer));
	g_iGroupId = StringToInt(sBuffer);
	
	(CVAR = CreateConVar("swgm_timer",		"120.0",	"Время между проверками на группу.")).AddChangeHook(OnTimeChange);
	...
	RegAdminCmd("swgm_list", CMD_List, ADMFLAG_ROOT);
...
public Action CMD_List(int iClient, int args)
{
	...
	if(GetCmdReplySource() == SM_REPLY_TO_CHAT)
		PrintToChat(iClient, "[SWGM] Результаты выведены в консоль!");
